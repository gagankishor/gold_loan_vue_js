  ...
  <b-container fluid>
    <form method="post" ref="form" @submit.prevent="submitForm">
      <div class="d-flex justify-content-between align-items-center ">
        <h4 class="mb-6">Edit Gold Loan</h4>
      ...
      <b-card  class="mt-3" header="Charges Information " bg="dark">
          ...
          <b-card class="mt-5" header="Upload Gold Item 1 Photo:">
            ...
            <label for="fileInput1" class="d-block upload-image mb-3">
              <img
                alt=""
                class="img-fluid"
                v-if="fileInput1" :src="fileInput1"
              />
              <input
                type="file"
                ref="fileInput1"
...
export default {
  ...
  computed: {
    ...
    dueAmount() {
      // Calculate the due amount based on the user input and the interest
  ...
  methods: {
    ...
    fetchdataall () {
      ...
      axios.get(`https://goldloanbackend.onrender.com/api/goldloan/${this.$route?.params?.id}`)
        .then(response => {
          ...
          console.log("gerenter",this.InterestType)
    ...
    submitForm() {
      axios.put(`https://goldloanbackend.onrender.com/api/goldloan/${this.$route?.params?.id}`, {        
        ...
        InterestType:this.InstanceType,
  ...
  watch: {
    // Whenever any of the input values change, recalculate the due amount
    // Member : 'fetchData1',
    EMIAmount : 'calculateInterest',
    roi: 'calculateInterest',
    Period: 'calculateInterest',
